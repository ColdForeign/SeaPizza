...
internal partial class UserService
{
    ...
    public async Task<string> AssignRolesAsync(string userId, UserRolesRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _userManager.IsInRoleAsync(user, SeaPizzaRoles.Admin)
            && request.UserRoles.Any(a => !a.Enabled && a.RoleName == SeaPizzaRoles.Admin))
        {
            // Get count of users in Admin Role
            int adminCount = (await _userManager.GetUsersInRoleAsync(SeaPizzaRoles.Admin)).Count;
            ...
            if (user.Email == UserConstants.DefaultAdmin.EmailAddress)
            {
